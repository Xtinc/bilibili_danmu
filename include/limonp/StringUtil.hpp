...
namespace limonp
{
  ...
  bool Utf8ToUnicode(const char *const str, size_t len, Uint16Container &vec)
  {
    ...
    for (size_t i = 0; i < len;)
    {
      ...
      else if ((uint8_t)str[i] <= 0xdf && i + 1 < len)
      { // 110xxxxxx
        ch1 = (str[i] >> 2) & 0x07;
  ...
  bool Utf8ToUnicode32(const string &str, Uint32Container &vec)
  {
    ...
    for (size_t i = 0; i < str.size();)
    {
      ...
      else if ((uint8_t)str[i] <= 0xdf && i + 1 < str.size())
      { // 110xxxxxx
        // 5bit, total 5bit
      ...
      else if ((uint8_t)str[i] <= 0xef && i + 2 < str.size())
      { // 1110xxxxxx
        // 4bit, total 4bit
      ...
      else if ((uint8_t)str[i] <= 0xf7 && i + 3 < str.size())
      { // 11110xxxx
        // 3bit, total 3bit
