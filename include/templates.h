...
namespace danmu
{
  namespace details
  {
    ...
    public:
      ...
      void commit() const noexcept { commit_ = true; } // const noexcept

    protected:
      ScopeGuardBase(ScopeGuardBase &&other) : commit_(other.commit_) { other.commit(); } // 拷贝构造函数
      ~ScopeGuardBase() {}
      mutable bool commit_; // mutable

    private:
      ScopeGuardBase &operator=(const ScopeGuardBase &) = delete; // 禁止赋值操作
    };
  }
  template <typename Func>
  class ScopeGuard : public details::ScopeGuardBase
  {
  public:
    ScopeGuard(Func &&func) : func_(func) {}      // 右值构造
    ScopeGuard(const Func &func) : func_(func) {} // const引用构造
    ~ScopeGuard()
    {
      if (!commit_)
        func_();
    }
    ScopeGuard(ScopeGuard &&other) : ScopeGuardBase(std::move(other)), func_(other.func_) {} // 移动构造函数，入参ScopeGuard&&，并且使用了move操作
  private:
    Func func_;
  };
